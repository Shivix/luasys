...
App {
    name = "zig",
    custom = function()
        local curl <close> = assert(io.popen([[curl -fsSL https://ziglang.org/download/index.json | jq -r '.master["x86_64-linux"].tarball']]))
        ...
        assert(os.execute(string.format("tar -xJf %q %q/zig --strip-components=1", untar_dir, dir_name)))
        assert(os.execute(string.format("tar -xJf %q %q/lib --strip-components=1", untar_dir, dir_name)))
